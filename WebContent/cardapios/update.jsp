...
<t:main title="Atualizar Item">
    ...
    <jsp:body>
    	<div class="row">
       		<h1 class="display-4">Atualizar Item</h1>
       		<hr/>
       	</div>
	            ...
	            <t:card width="100%" title="Item" subtitle="Informações do item do cardápio" classes="mt-5">
        			<jsp:body>
        				<div class="form-group">
			                ...
			                <input class="form-control" id="nm_item" name="nm_item" required type="text" value="${item.nm_item}" />
			            </div>
			            
			            <div class="form-group">
			                <label class="control-label" for="ds_item">Descrição</label>
			                <textarea class="form-control" id="ds_item" name="ds_item" required maxlength="250">${item.ds_item}</textarea>
			            </div>
			            
			            <div class="form-group">
			                <label class="control-label" for="vl_preco">Preço</label>
			                <input class="form-control" id="vl_preco" data-inputmask="'alias': 'currency', 'groupSeparator': '.', 'autoGroup': true, 'digits': '2', 'greedy': 'false', 'placeholder': '0,00', 'radixPoint': ','" pattern="^\s*(?:[1-9]\d{0,2}(?:\.\d{3})*|0)(?:,\d{1,2})?$" name="vl_preco" required type="text" value="${item.vl_preco}" />
			            </div>
			            ...
			            <div class="form-group">
			                ...
			                <select class="form-control" id="item_categoria" name="item_categoria">
			                	...
			                	<c:forEach items="${categorias}" var="categoria">
									<option ${item.item_categoria.id_categoria_item == categoria.id_categoria_item ? 'selected="selected"' : ''} value="${categoria.id_categoria_item}">
										${categoria.nm_categoria_item}
									</option>
			                	</c:forEach>
			                </select>
			            </div>
